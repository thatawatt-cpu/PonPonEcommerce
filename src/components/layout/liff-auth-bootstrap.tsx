...
import { getLiffTokens, initLiff, isLiffLoggedIn } from "@/lib/liff";
...
async function bootstrapLineSession(): Promise<void> {
  ...
  if (!isLiffLoggedIn()) {
    console.info("[ponpon-auth] LINE session not ready yet; skipping auto login");
    return;
  }
  ...
  if (expired && !PONPON_SKIP_LINE_LIFF && !recentlyAttempted) {
    console.info("[ponpon-auth] idToken expired — skipping auto re-login");
  ...
  try {
    ...
    if (is401 && !PONPON_SKIP_LINE_LIFF && !recentlyAttempted) {
      console.info("[ponpon-auth] token rejected by backend — skipping auto re-login");
      localStorage.setItem(REAUTH_KEY, String(Date.now()));
      return;
    }
